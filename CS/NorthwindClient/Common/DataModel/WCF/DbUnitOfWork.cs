...
namespace NorthwindClient.Common.DataModel.WCF {
    public abstract class DbUnitOfWork<TContext> : UnitOfWorkBase, IUnitOfWork where TContext : DataServiceContext {
        ...
        protected IRepository<TEntity, TPrimaryKey>
            GetRepository<TEntity, TPrimaryKey>(Expression<Func<TContext, DataServiceQuery<TEntity>>> dbSetAccessor, Expression<Func<TEntity, TPrimaryKey>> getPrimaryKeyExpression)
        ...
        protected IReadOnlyRepository<TEntity>
            GetReadOnlyRepository<TEntity>(Func<TContext, DataServiceQuery<TEntity>> dbSetAccessor)
            where TEntity : class {
