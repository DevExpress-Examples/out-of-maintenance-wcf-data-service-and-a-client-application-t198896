...
namespace NorthwindClient.Common.ViewModel {
    ...
    public abstract class CollectionViewModelBase<TEntity, TProjection, TPrimaryKey, TUnitOfWork> : ReadOnlyCollectionViewModel<TEntity, TProjection, TUnitOfWork>
        ...
        public virtual void Edit(TProjection projectionEntity) {
            if (Repository.IdDetached(projectionEntity))
                return;
        ...
        public virtual void Save(TEntity entity) {
            try {
                Repository.UnitOfWork.SaveChanges();
                Messenger.Default.Send(new EntityMessage<TEntity, TPrimaryKey>(Repository.GetPrimaryKey(entity), EntityMessageType.Changed));
            }
            catch (DbException e) {
                MessageBoxService.ShowMessage(e.ErrorMessage, e.ErrorCaption, MessageButton.OK, MessageIcon.Error);
            }
